...
async function tournamentList() {
  try {
    const req = await fetch(
      "https://alihan-myproject.azurewebsites.net/api/v1/app/tournament/tourney/completed",
      {
        method: "GET",
        headers: {
...
export default function EndedTournaments() {
          ...
          return (
            <div className={divClass} key={`${elem.name}=${index}`}>
              ...
              <div className="viewTournaments">
                <a href={`/completed/${elem.id}`}>View</a>
              </div>
              {isAdmin && (
                <button
  ...
  return <div className="tournamentInfo">No ended tournaments</div>;
}
